function [ chart ] = createLineWeekRunsPPH( alldata, lineNum )
...
x = 1:1:length(data.(char(lineName)).PPH);
barchart = bar(x, y);
...
titlestr = sprintf('%s Line - Week Runs PPH', char(lineName));
