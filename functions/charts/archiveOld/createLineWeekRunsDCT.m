function [ chart ] = createLineWeekRunsDCT( alldata, lineNum )
...
x = 1:1:length(data.(char(lineName)).DCT);
barchart = bar(x, y);
...
titlestr = sprintf('%s Line - Week Runs DCT', char(lineName));
